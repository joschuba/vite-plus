...
Set it when detection cannot select one provider. The main case is a tool migration, where the old and the new marker coexist until the switch completes. While both markers coexist, `doc.provider` selects which one runs. Remove it when you remove the old marker. The selected provider's marker must also be declared in `package.json` (any dependency field, `peerDependencies` included) and installed, so a typo, a stale value, or a transitive-only package makes `vp doc` fail before it runs the tool.
